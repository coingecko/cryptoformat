  ...
  describe("is BTC or ETH", () => {
    ...
    describe("raw = false", () => {
      test("returns currency with ISO Code", () => {
        expect(formatCurrency(0.0, "BTC", "en")).toBe("Ƀ 0");

        // Large cyrpto, no decimals
        expect(formatCurrency(1001, "BTC", "en")).toBe("Ƀ 1,001");

        // Medium cyrpto, 3 decimals
        expect(formatCurrency(51.1, "BTC", "en")).toBe("Ƀ 51.100");

        // Small cyrpto, 6 decimals
        expect(formatCurrency(11.1, "BTC", "en")).toBe("Ƀ 11.100000");
        expect(formatCurrency(9.234, "ETH", "en")).toBe("Ξ 9.234000");

        // Very small cyrpto, 8 decimals
        expect(formatCurrency(0.5, "BTC", "en")).toBe("Ƀ 0.50000000");
      });
    });
  });
